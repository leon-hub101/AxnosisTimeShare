...
Set this in the app via the /login page or manually in Preferences:

node -e "require('@capacitor/preferences').Preferences.set({ key: 'currentUser', value: JSON.stringify({ email: 'John.Doe@example', role: 'admin' }) }).then(() => console.log('Admin user set'))". Note that the role based functionality still needs work accross the application, but the John Doe login will give admin access. 
...
src/app/routes.ts: Defines lazy-loaded routes for `施行
